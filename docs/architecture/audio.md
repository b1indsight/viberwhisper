...
2. Drops the stream.
3. Writes remaining samples to WAV. If chunking is enabled and complete unflushed chunks remain, writes them as chunk files before the final tail instead of collapsing them into one large tail file.
