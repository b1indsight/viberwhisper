...
1. Toggle 模式长录音在**录音过程中按阈值封片并后台上传转写**，停止录音时仅 flush 尾片并等待结果收敛。
2. 已存在的长 WAV 文件（包括批量转写场景）支持**离线自动分片**，依次上传后将结果拼接返回。
3. 单片上传遇到网络错误或 5xx 时，支持**指数退避自动重试**，次数可配置。
4. 分片、重试与结果收敛逻辑对调用方完全透明：`Transcriber::transcribe(&self, wav_path: &str)` 签名不变。
5. 新增三个可选配置项，有合理默认值，旧配置无需修改。

## 非目标

- **真正的流式 ASR**（边录音边转写、实时字幕流）：本期不涉及。
  - 本方案仍以“分片文件 → 分片转写 → 停止后统一收敛文本”为边界，不做 token 级/句级实时增量输出，也不引入实时字幕 UI。
- **多分片并发上传**：串行上传已能满足需求，并发需引入 `tokio` 或额外调度复杂度，当前 codebase 使用 `reqwest::blocking`，不值得现阶段引入异步运行时。
...
record()
  ...
  │    ├─ flush_chunk_to_wav()
  │    └─ enqueue_background_transcription(chunk)
  └─ stop_recording()
       ├─ flush_tail_chunk_if_needed()
       └─ wait_for_all_results() → merge_texts(results, language)
...
- **双路径设计**：Toggle 长录音走“录音时封片 + 后台转写”，已有 WAV 文件继续走离线分片路径，避免把导入文件场景硬塞进录音线程。
...
| `src/audio/recorder.rs` | **修改** | Toggle 录音达到阈值时封片落盘，并把分片转写任务交给后台队列 |
| `src/transcriber/api.rs` | **修改** | `ApiTranscriber` 新增三个字段，`transcribe` 增加离线分派逻辑，新增私有上传/重试方法 |
| `src/core/config.rs` | **修改** | `AppConfig` 新增三个带默认值的字段，更新 `get_field`/`set_field`/`apply_json` |
| `src/core/*` 或调度模块 | **修改** | 增加后台上传队列、结果收敛与停止录音时的等待逻辑 |

`Transcriber` trait、`MockTranscriber`、`factory.rs`、`main.rs` **接口层面不需要修改**；`tray.rs` / `hotkey.rs` 仅在需要显示进度时才评估是否触及。
...
### Phase 3 — Toggle 录音后台上传链路

1. 在录音侧增加“达到阈值即封片落盘”的能力
2. 增加后台上传队列与结果收敛容器
3. 实现停止录音时 flush 尾片并等待所有后台任务完成
4. 写录音链路单元/集成测试，验证顺序、清理与失败传播

**验收**：长录音停止后只需等待尾片和未完成任务收敛；短录音行为与改动前一致。

### Phase 4 — 转写器离线路径与重试逻辑
...
5. 改写 `transcribe` 入口，供已有 WAV / 批量转写场景按需分派到 `split_wav`
6. 写 mock HTTP 单元测试
...
- **真正的流式 ASR**：在当前“分片文件级后台转写”基础上，继续演进到句级/增量级实时输出。
- **多分片并发上传**：引入 `rayon` 或独立 worker 并行处理 `chunks`，需处理结果有序合并；仅在串行延迟成为实际瓶颈后评估。
