...
mod tests {
    ...
    use crate::core::orchestrator::{OrchestratorConfig, SessionError, SessionOrchestrator};
    use crate::input::typer::{MockTyper, TextTyper};
    use crate::session::SessionId;
    use crate::transcriber::{MockTranscriber, Transcriber};
    use std::sync::Arc;
    ...
    fn test_full_pipeline_mock() {
        ...
        let typer = MockTyper;
        let chunk = WavChunk::from_encoded_bytes(b"fake wav".to_vec());
    ...
    fn test_orchestrator_integration_single_chunk() {
        ...
        let orchestrator =
            SessionOrchestrator::new(transcriber, OrchestratorConfig::new(Some("en".to_string())));

        orchestrator.start_session(SessionId(1)).unwrap();
        let chunk = WavChunk::from_encoded_bytes(b"fake wav".to_vec());
        let _ = orchestrator.on_chunk_ready(SessionId(1), chunk);
        let result = orchestrator.finish_session(SessionId(1));
    ...
    fn test_orchestrator_no_chunks() {
        let transcriber: Arc<dyn Transcriber> = Arc::new(MockTranscriber);
        let orchestrator = SessionOrchestrator::new(transcriber, OrchestratorConfig::new(None));
